...
void countNodes(Task *root, int *contagem) {
...
void saveTreeInVectorPreOrder(Task* root, Task **vector, int *index) {
...
void finishExecution(BST *tree,  FILE *fp) {
    ...
    index = 0;
    saveTreeInVectorPreOrder(tree->root, vector, &index);
    for (index = 0; index < contagem-1; index++) {
        fprintf(fp,"%s\n", vector[index]->name);
        fprintf(fp,"%d\n",vector[index]->prioridade);
        fprintf(fp,"%d/%d\n",vector[index]->entrega.day, vector[index]->entrega.month);
    }
    fprintf(fp,"%s\n", vector[index]->name);
    fprintf(fp,"%d\n",vector[index]->prioridade);
    fprintf(fp,"%d/%d",vector[index]->entrega.day, vector[index]->entrega.month);
